#!/usr/bin/env python3
"""Post-presentation statistical and resource analyses for CMP611 DNABERT-2 project.

Inputs are CSV files generated by the existing final aggregation workflow.
The statistical comparison is intentionally conservative because there are only
three seeds. It reports paired deltas against the 100% condition and flags
practically meaningful drops instead of over-claiming formal significance.
"""
...
def write_markdown(stats: pd.DataFrame, thresholds: pd.DataFrame, out_md: Path) -> None:
    ...
    lines = [
        "# Post-Presentation Statistical Analysis",
        "",
        "Comparison target: each lower-data run is paired with the 100% run from the same task and seed.",
        "",
        "Important limitation: only three seeds are available, so this should be presented as a paired effect-size and bootstrap-CI analysis, not as a strong formal hypothesis test.",
        "",
...
def main() -> int:
    ...
    stats_csv = out_dir / "post_presentation_paired_drop_analysis.csv"
    threshold_csv = out_dir / "post_presentation_threshold_summary.csv"
    report_md = out_dir / "post_presentation_statistical_analysis.md"
